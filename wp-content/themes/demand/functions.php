<?php
	...
	//add_image_size( 'demand', 300, 300, true ); //(cropped)
	...
	//register_nav_menus(array('archetype' => 'Archetype'));
		...
		function dmd_widgets_init() {
		    register_sidebar( array(
		        'name' => __( 'Business Owner Testimonial', 'dmd' ),
		        'id' => 'business-owner',
				'description' => __( 'Business owner testimonial', 'dmd' ),
		...
		add_action( 'widgets_init', 'dmd_widgets_init' );
	...
	add_action( 'login_enqueue_scripts', 'dmd_login_logo' );

	function dmd_login_logo_url() {
		    return get_bloginfo( 'url' );
		}
	add_filter( 'login_headerurl', 'dmd_login_logo_url' );
    ...
    /* Typekit */
    function dmd_theme_typekit() {
	    wp_enqueue_script( 'theme_typekit', '//use.typekit.net/cqa2gsc.js');
		}
	add_action( 'wp_enqueue_scripts', 'dmd_theme_typekit' );

	function dmd_theme_typekit_inline() {
		  if ( wp_script_is( 'theme_typekit', 'done' ) ) { ?>
		  	<script type="text/javascript">try{Typekit.load();}catch(e){}</script>
		<?php }
		}
	add_action( 'wp_head', 'dmd_theme_typekit_inline' );    
